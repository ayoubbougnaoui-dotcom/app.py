...
st.markdown("<div class='subtitle'>Créez un CV au design moderne, dynamique et personnalisé</div>", unsafe_allow_html=True)
...
with col_style2:
    police = st.selectbox(
        "Style d'écriture (Police) :",
        ["Moderne (Helvetica / Arial)", "Classique Pro (Times / Serif)"]
    )
...
font_family = "'Helvetica Neue', Helvetica, Arial, sans-serif" if "Moderne" in police else "'Times New Roman', Times, serif"
...
with st.expander("🏢 3. Expériences Professionnelles (Laisse vide si inutile)", expanded=False):
    st.markdown("#### 🔹 Expérience 1 (Obligatoire)")
    ...
    st.markdown("---")
    st.markdown("#### 🔹 Expérience 2 (Optionnelle)")
    ...
    st.markdown("---")
    st.markdown("#### 🔹 Expérience 3 (Optionnelle)")
    exp3_poste = st.text_input("Poste 3", value="")
    exp3_entreprise = st.text_input("Club / Entreprise 3", value="")
    exp3_dates = st.text_input("Dates 3", value="")
    exp3_missions = st.text_area("Missions 3", value="")

# --- SECTION 4 : FORMATIONS ---
with st.expander("🎓 4. Formations & Diplômes (Laisse vide si inutile)", expanded=False):
    st.markdown("#### 🎓 Formation 1 (Obligatoire)")
    ...
    st.markdown("---")
    st.markdown("#### 🎓 Formation 2 (Optionnelle)")
    diplome2 = st.text_input("Diplôme 2", value="Formation Management Sportif & Leadership")
    ecole2 = st.text_input("École 2", value="The Real Academy")
    dates2 = st.text_input("Année 2", value="2020")

    st.markdown("---")
    st.markdown("#### 🎓 Formation 3 (Optionnelle)")
    diplome3 = st.text_input("Diplôme 3", value="")
    ecole3 = st.text_input("École 3", value="")
    dates3 = st.text_input("Année 3", value="")

# --- SECTION 5 : SKILLS & LANGUES AVEC ÉTOILES ---
with st.expander("🛠️ 5. Compétences & Langues", expanded=False):
    ...
    st.markdown("#### 🗣️ Langues parlées & Niveaux (Sélectionne)")
    col_l1, col_l2 = st.columns(2)
    with col_l1:
        langue1_nom = st.text_input("Langue 1", value="Français")
        langue1_lvl = st.slider("Niveau de la Langue 1 (Étoiles)", 1, 5, 5)
    with col_l2:
        langue2_nom = st.text_input("Langue 2", value="Espagnol")
        langue2_lvl = st.slider("Niveau de la Langue 2 (Étoiles)", 1, 5, 4)
...
if st.button("✨ Générer mon CV Personnalisé"):
    if not nom or not titre_pro or not email:
        ...
        # --- GESTION DYNAMIQUE DES EXPÉRIENCES (ANTI-BLOC VIDE) ---
        experiences_html = f"""
        ...
        """
        if exp2_poste and exp2_entreprise:
            experiences_html += f"""
            ...
            """
        if exp3_poste and exp3_entreprise:
            experiences_html += f"""
        ...
        # --- GESTION DYNAMIQUE DES FORMATIONS (ANTI-BLOC VIDE) ---
        formations_html = f"<p><strong>{diplome1}</strong> – {ecole1} <span style='color:#666;'>({dates1})</span></p>"
        if diplome2 and ecole2:
            formations_html += f"<p style='margin-top:8px;'><strong>{diplome2}</strong> – {ecole2} <span style='color:#666;'>({dates2})</span></p>"
        if diplome3 and ecole3:
            formations_html += f"<p style='margin-top:8px;'><strong>{diplome3}</strong> – {ecole3} <span style='color:#666;'>({dates3})</span></p>"

        # --- GESTION DES LANGUES AVEC ÉTOILES ---
        stars1 = "🌟" * langue1_lvl
        stars2 = "🌟" * langue2_lvl if langue2_nom else ""
        langues_html = f"<span>{langue1_nom} ({stars1})</span>"
        if langue2_nom:
            langues_html += f" &nbsp;|&nbsp; <span>{langue2_nom} ({stars2})</span>"

        # --- RENDU DE LA PAGE HTML FINALE ---
        cv_html_page = f"""
        <html>
        <head>
            <meta charset="UTF-8">
            <style>
                body {{ font-family: {font_family}; color: #333; background-color: #ffffff; padding: 10px; }}
        ...
        <body>
            <div class="cv-container">
                ...
                {"<h4>📖 Profil & Objectifs</h4><p>" + accroche + "</p>" if accroche else ""}
                ...
                <p><strong>Langues :</strong> {langues_html}</p>
                
                {f"<h4>⚽ Centres d'intérêt</h4><p>{interets}</p>" if interets else ""}
            </div>
        </body>
        </html>
        """
        ...
        components.html(cv_html_page, height=800, scrolling=True)
